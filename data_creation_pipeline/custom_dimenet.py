...
class DimeNet(torch.nn.Module):
    ...
    def from_qm9_pretrained(
        ...
        perm = torch.from_numpy(random_state.permutation(np.arange(130831)).astype(np.int64))
    ...
    def forward(
        ...
        for interaction_block, output_block in zip(self.interaction_blocks,
            ...
            P = torch.cat([P, output_block(x, rbf, i, num_nodes=pos.size(0))], dim=-1)
        if batch is None:
            
            return P.sum(dim=0)
        else:
            # print("sjahdkjashdjash" ,scatter(P, batch, dim=0, reduce='sum').shape)

            return scatter(P, batch, dim=0, reduce='sum')
            # P_summed = scatter(P, batch, dim=0, reduce='sum', dim_size=batch.max().item() + 1)
            # return P_summed
            # P_dense, mask = to_dense_batch(P, batch=batch)
            return P_dense
