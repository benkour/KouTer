...
path = os.path.join(os.getcwd(), 'qm9_data', 'QM9')
dataset = QM9(path)
print('Loading model weights...')
with open('graph_parameters.json', 'r') as json_file:
    config = json.load(json_file)
current_property = config["property"]
dimenet_target_list = config["dimenet_target"]
VOC_prediction_flag = config["VOC_prediction_flag"]
# suppress warnings
warnings.filterwarnings("ignore")
# prediction_target = 'dP'

def add_dimenet_outputs():
    for prediction_target in current_property:
                graph_data_path = os.getcwd() + '/graph_out/'+str(prediction_target) + '_graphs/'
                ...
                for dimenet_target in dimenet_target_list:
                    ...
                    print("non solvent graphs with DimeNet target", dimenet_target, "done.")
                    if VOC_prediction_flag:
                        graph_data_path = os.getcwd() + '/graph_out/'+str(prediction_target) + '_solvent_graphs/'
                        print(graph_data_path)
                        graph_files = sorted(glob.glob(os.path.join(graph_data_path, '*.pt')))
                        graphs = [torch.load(f) for f in graph_files]
