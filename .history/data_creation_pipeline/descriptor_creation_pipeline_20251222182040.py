...
from PaDEL_pywrapper.descriptor import FMF, RingCount, MLFER, Autocorrelation, EStateAtomType, ExtendedTopochemicalAtom, HBondDonorCount
...
def add_descriptors_to_graphs():
    ...
    graph_targets = ["AN"]
    for i in range(len(graph_targets)):
        graph_targets[i] = graph_targets[i] + '_graphs'
    #ADD graphs to the graph targets list if there are more than one
    ...
    for target in graph_targets:
        ...
        target_values = np.array([graph.to_dict()[target_name].item() for graph in graphs])
        ...
        if os.path.exists(solvent_desc_path):
            ...
            print(f"Final number of descriptors for solvent with target {target_name}: {graph_descriptors.shape[1]}")
