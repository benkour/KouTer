...
def add_descriptors_to_graphs(property):
    ...
    #keep only graph targets that contain the property string
    graph_targets = [gt for gt in graph_targets if property in gt]
    # Put solvent graphs first because we add all other descriptors to solvent graphs at the end
    graph_targets.sort(key=lambda x: 1 if x.startswith(solvent_graphs_name) else 0)
    print("Graph targets to process:", graph_targets)
    for target in graph_targets:
        ...
        if os.path.exists(os.path.join(desc_out_path, f'processed_graphs_{target}.pkl')):
            ...
            processed_graph_descriptors = save_path[]
        ...
        if os.path.exists(solvent_desc_path):
            ...
            solvent_descriptors = np.arrasolvent_output_pathy(solvent_descriptors).astype(float)
